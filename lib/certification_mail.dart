...
class _CertificationMailState extends State<CertificationMail> {
  ...
  final List<FocusNode> _childFocusNodes = List.generate(
    kCodeLength,
    (_) => FocusNode(),
  );
          ...
          if (response.statusCode == 200) {
            if (data['result'] == "OK") {
              ...
              try {
                ...
                final updated = current.copyWith(email: widget.email);
              ...
              setState(() {
                _errorMessage = ErrorMessage.instance.exceptionMailRegist(
                  data['reason'],
                );
              });
            }
          ...
          if (response.statusCode == 200) {
            ...
            if (data['result'] == "OK") {
              ...
              try {
                ...
                final serverUuid =
                    (data['uuid'] is String) ? (data['uuid'] as String) : null;
                final serverUserId =
                    (data['user_id'] is int)
                        ? data['user_id'] as int
                        : int.tryParse(data['user_id']?.toString() ?? '') ?? 0;
                final updated = current.copyWith(
                  userId: serverUserId > 0 ? serverUserId : current.userId,
                  email: widget.email,
                  uuid:
                      (serverUuid != null && serverUuid.isNotEmpty)
                          ? serverUuid
                          : current.uuid,
                  status: 'verified',
              ...
              setState(() {
                _errorMessage = ErrorMessage.instance.exceptionUserRegist(
                  data['reason'],
                );
              });
            }
