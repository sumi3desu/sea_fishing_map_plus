...
class _PremiumPageState extends ConsumerState<PremiumPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (mounted && !_inited) {
        _inited = true;
        try { await ref.read(premiumStateProvider.notifier).initialize(); } catch (_) {}
  ...
  Future<void> _purchasePackage(Package pkg) async {
    ...
    try {
      ...
      await _loadActiveProductId();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('購入が完了しました')));
      ...
      if (code == PurchasesErrorCode.purchaseCancelledError) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('購入エラー: $code')));
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('購入エラー: $e')));
    } finally {
  ...
  Widget _statusTag(bool isPurchased) {
    ...
    final Color bg = isPurchased ? purchasedColor.withOpacity(0.08) : Colors.grey.withOpacity(0.06);
            ...
            child: Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
              Text(
                'プレミアム状態: ' + (st.isPremium ? '購入済み' : (st.isLoading ? '読み込み中' : '未購入')),
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600),
              ),
              const SizedBox(height: 8),
              if ((st.errorMessage ?? '').isNotEmpty)
                Text('エラー: ${st.errorMessage}', style: const TextStyle(color: Colors.red)),
              const SizedBox(height: 12),
              const Divider(height: 1),
              const SizedBox(height: 12),
              const Text('提供中のプラン', style: TextStyle(fontWeight: FontWeight.w600)),
              ...
              FutureBuilder<Offerings>(
                ...
                builder: (context, snap) {
                  ...
                  if (offerings.current != null) packages.addAll(offerings.current!.availablePackages);
                  for (final o in offerings.all.values) {
                    for (final p in o.availablePackages) {
                      if (!packages.any((e) => e.identifier == p.identifier)) packages.add(p);
                    }
                  }
                  if (packages.isEmpty) return const Text('現在、提供中のプランはありません');
                  return Column(
                    children: packages.map((p) {
                      final sp = p.storeProduct;
                      final bool isPurchased = (_activeProductId != null && _activeProductId == sp.identifier);
                      return Card(
                        elevation: 0,
                        ...
                        child: InkWell(
                          borderRadius: BorderRadius.circular(12),
                          onTap: _purchasing ? null : () => _purchasePackage(p),
                          child: Padding(
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    const Spacer(),
                                    _priceTag(_purchasing, '${sp.priceString} で購入'),
                                  ],
                                ),
                              ],
                            ),
                  ...
                  ElevatedButton.icon(
                    onPressed: st.isLoading
                        ...
                        : () async {
                            ...
                            final err = ref.read(premiumStateProvider).errorMessage;
                  ...
                  ElevatedButton.icon(
                    onPressed: st.isLoading
                        ...
                        : () async {
                            ...
                            if ((s.errorMessage ?? '').isNotEmpty && context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('復元エラー: ${s.errorMessage}')),
                              );
