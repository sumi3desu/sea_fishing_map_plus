...
Future<int> getCountRows(Database localDb, String tableName) async {
  final count =
      Sqflite.firstIntValue(
        await localDb.rawQuery('SELECT COUNT(*) FROM $tableName '),
      )!;
  return count;
}

Future<void> initialVersionItem(
  Database db,
  bool initialize,
  int userId,
  String name,
...
Future<void> matchTheTable(Database localDb, int userId) async {
  try {
    ...
    if (resp.statusCode == 200 && data['status'] == 'success') {
      ...
      for (int i = 0; i < versions.length; i++) {
        ...
        for (int ii = 0; ii < localVersions.length; ii++) {
          ...
          if (version['name'] == localVersion['name']) {
            logPrint(
              'name[${version['name']}] version[${version['version']}] localVersion[${localVersion['version']}]',
            );
      ...
      logPrint('バージョンの取得に失敗しました: ステータス異常');
    }
  } catch (e) {
    logPrint('バージョンの取得に失敗しました: $e');
  }
}

Future<List<Map<String, Object?>>> getVersions(
  Database localDb,
  int userId,
) async {
