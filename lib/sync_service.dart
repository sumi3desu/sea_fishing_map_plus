...
class SioSyncService {
  ...
  Future<bool> syncFishingData({required int userId, bool force = false}) async {
    try {
      ...
      if (remote.isEmpty) {
        ...
        if (!okKubun) lastError = 'get_kubun.php';
        else if (!okTeibou) lastError = 'get_teibou.php';
        else if (!okTodou) lastError = 'get_todoufuken.php';
      ...
      Future<bool> _maybeSync(String name, Future<bool> Function() action) async {
        ...
        if (need) {
          ...
          if (ok && rv != null) {
            ...
            // 同期失敗のエンドポイントを記録
            if (name == 'kubun') lastError = 'get_kubun.php';
            else if (name == 'teibou') lastError = 'get_teibou.php';
            else if (name == 'todoufuken') lastError = 'get_todoufuken.php';
          }
          return ok;
        }
        return true;
      ...
      final okTeibou = await _maybeSync('teibou', () => _syncTeibou(userId: userId));
  ...
  Future<bool> _syncTeibou({required int userId}) async {
    ...
    return await db.transaction((txn) async {
      ...
      for (final r in rows) {
        ...
        if ((registrantName ?? '').isNotEmpty) rowMap['registrant_name'] = registrantName;
        if (createAt != null && createAt.isNotEmpty) rowMap['create_at'] = createAt;
      ...
      lastError = 'get_teibou.php DB反映失敗';
      return false;
    });
  }
  ...
  Future<bool> _syncKubun() async {
    ...
    dynamic decoded;
    try { decoded = jsonDecode(resp); } catch (_) {
  ...
  Future<String?> _post(Uri uri, {required String body, String contentType = 'application/x-www-form-urlencoded'}) async {
    try {
      ...
      final ep = uri.pathSegments.isNotEmpty ? uri.pathSegments.last : uri.toString();
