...
try {
    ...
    if ($action === 'regist') {
        // いいねのトグル実装
        // テーブル: post_thumb(post_id INT, user_id INT, created_at DATETIME)
        $pdo->beginTransaction();
        $stmt = $pdo->prepare('SELECT 1 FROM post_thumb WHERE post_id = ? AND user_id = ? LIMIT 1');
        $stmt->execute([$post_id, $user_id]);
        $exists = ($stmt->fetchColumn() !== false);
        if ($exists) {
            $del = $pdo->prepare('DELETE FROM post_thumb WHERE post_id = ? AND user_id = ?');
            $del->execute([$post_id, $user_id]);
            $resAction = 'remove';
        } else {
            $ins = $pdo->prepare('INSERT INTO post_thumb (post_id, user_id, created_at) VALUES (?, ?, NOW())');
            $ins->execute([$post_id, $user_id]);
            $resAction = 'insert';
        }
        $cntStmt = $pdo->prepare('SELECT COUNT(*) FROM post_thumb WHERE post_id = ?');
        $cntStmt->execute([$post_id]);
        $count = intval($cntStmt->fetchColumn());
        $pdo->commit();
    } else {
        // get: 件数のみ返却
        $stmt = $pdo->prepare('SELECT COUNT(*) FROM post_thumb WHERE post_id = ?');
