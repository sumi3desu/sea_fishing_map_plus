...
try {
    ...
    // ユーザ単位の投稿ブロックを確認
    $stUser = $pdo->prepare('SELECT posts_blocked, posts_blocked_until FROM `user` WHERE user_id = ? LIMIT 1');
    ...
    if ($userRow) {
        ...
        if ($postsBlocked) {
            ...
            echo json_encode([
                'status' => 'error',
                'message' => '投稿の送信は停止中です。不適切な利用が確認されました。'
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
        if ($postsTempBlocked) {
            ...
            echo json_encode([
                'status' => 'error',
                'message' => '投稿の送信は一時停止中です。不適切な利用が確認されました。'
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
    }
