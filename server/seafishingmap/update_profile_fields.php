...
try {
    ...
    $nickName = isset($_POST['nick_name']) ? trim((string)$_POST['nick_name']) : '';
    $xUsername = isset($_POST['x_username']) ? trim((string)$_POST['x_username']) : '';
    $xPublic = isset($_POST['x_public']) ? (int)$_POST['x_public'] : 0;
    $instagramUsername = isset($_POST['instagram_username']) ? trim((string)$_POST['instagram_username']) : '';
    $instagramPublic = isset($_POST['instagram_public']) ? (int)$_POST['instagram_public'] : 0;
    ...
    if (mb_strlen($xUsername) > 15) {
        $xUsername = mb_substr($xUsername, 0, 15);
    }
    if (mb_strlen($instagramUsername) > 30) {
        $instagramUsername = mb_substr($instagramUsername, 0, 30);
    }
    ...
    $stmt->execute([
        ':nick_name' => $nickName,
        ':x_username' => ($xUsername !== '') ? $xUsername : null,
        ':x_public' => ($xPublic !== 0) ? 1 : 0,
        ':instagram_username' => ($instagramUsername !== '') ? $instagramUsername : null,
        ':instagram_public' => ($instagramPublic !== 0) ? 1 : 0,
        ':user_id' => $userId,
    ]);
